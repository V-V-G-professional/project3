...
class AnimalsAddPage(LoginRequiredMixin, DataMixin, CreateView):
...
class AnimalsShowPost(DataMixin, DetailView):
    model = Animals
...
class AnimalsShowCategory(DataMixin, ListView):
    paginate_by = 3
    model = Animals
    template_name = 'animals/index.html'
    context_object_name = 'posts'
    ...
    def get_queryset(self):
        return Animals.objects.filter(cat__slug=self.kwargs['cat_slug'], is_published=True)
